...
**Heartbeat**: `scripts/aria_heartbeat.py` (Mac mini) checks pending rows plus abandoned `picked_up` rows older than 15 minutes via bare REST count queries. Zero rows costs zero tokens and invokes no model. When work is waiting, the script atomically claims the oldest batch and includes that exact batch in an `openclaw system event --mode now` wake, so the session cannot observe a count and then omit the claim. Queue payloads are explicitly labelled untrusted operational data in the system event. The supplied launchd plist runs this check every five minutes and survives reboots. After a successful event, a local 20-minute cooldown suppresses duplicate wake events while the current session is working; an empty queue clears that cooldown immediately. If the wake fails, the script releases the claimed rows back to `pending` immediately, with the database's 15-minute visibility timeout as a final fallback.
