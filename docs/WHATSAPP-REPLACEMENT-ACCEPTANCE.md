...
| 6. Meeting Mode and intelligent filing | **PENDING** | Draft PR #48 contains migration 103, silent capture, checkpointing, shared draft review, recorder-only capture/discard control, event-specific ambiguity detection, destination revalidation, explicit filing and audit safeguards. The private recording namespace is database-confined and immutable after upload; the finish boundary verifies the stored byte count and MP4/WebM signature before Aria can receive a local-Whisper source URL. Automated checks pass. | Apply migration 103 and its verifier after the preceding stacked migrations, merge/deploy the reviewed stack, update/restart the Mac MCP runtime, then pass one lead consultation, one active-project meeting and one ambiguous-destination meeting (including two nearby events for the same project). |
