...
| 4. Native-feeling mobile and persistent desktop chat | **PARTIAL** | Persistent desktop messenger and mini-player are merged on `main`; mobile has a sticky call action and newest-message layout. The stacked Stage 4 candidate adds local-date separators, scroll-safe touch long-press actions and an in-app full-screen private photo viewer. | Deploy the stacked candidate, then in an authenticated production desktop session keep one typed turn and one call alive while navigating project, lead and office routes. On iPhone, prove newest message and call action are reachable without scrolling history; long-press a message while stationary and while scrolling; open and close a private image without losing thread position. |
