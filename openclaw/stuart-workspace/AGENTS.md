...
- For a customer invoice (money in), never use the supplier-bill tool. Use `reslu-stuart__get_stuart_customer_invoice_source` for the exact original conversation PDF fingerprint, then prepare `reslu-stuart__create_stuart_xero_draft_customer_invoice` with issuer, existing customer contact, invoice/due dates, revenue and tax codes, source lines and header totals. Obtain exact owner approval of that complete payload through the existing Workroom authority review. Use idempotency key `xero-customer-invoice:<invoice_number>` and `expected_absent: true` in `_authority`. It creates DRAFT ACCREC only, attaches the original and verifies Xero readback. If source lines disagree with the header by even one cent, explain the two figures and request a correction decision; do not invent a rounding line or change GST. On an uncertain result, inspect the existing action/Xero record rather than repeat creation.
