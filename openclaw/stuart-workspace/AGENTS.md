...
- For a customer invoice (money in), never use the supplier-bill tool. Use `reslu-stuart__get_stuart_customer_invoice_source` for the exact original conversation PDF fingerprint, then prepare `reslu-stuart__create_stuart_xero_draft_customer_invoice` with issuer, existing customer contact, invoice/due dates, revenue and tax codes, source lines and header totals. Obtain exact owner approval of that complete payload through the existing Workroom authority review. Use idempotency key `xero-customer-invoice:<invoice_number>` and `expected_absent: true` in `_authority`. It creates DRAFT ACCREC only, attaches the original and verifies Xero readback. On an uncertain result, inspect the existing action/Xero record rather than repeat creation.
- Phillip's standing rule: the invoice already sent to the client is gospel. Preserve that issued PDF, its total and GST. When the human or traceable sending evidence confirms this exact invoice was sent, include `issued_to_client: true` and the unchanged source figures in the approved tool payload. For a one-cent net-line/subtotal discrepancy only, the server reconciles the largest positive net line (first on ties) to the issued total minus issued GST and records before/after amounts. Show the reconciliation in the draft review and report it; do not ask Phillip which invoice to correct for this case. Never pre-adjust the supplied source lines, invent a balancing charge, change GST, or regenerate/re-send the issued invoice. Larger discrepancies or conflicting tax amounts still need review. This rule does not bypass exact approval of the draft, duplicate checks or readback.
...
- The only permitted accounting writes are a source-backed, explicitly approved supplier contact with no bank details, a Xero DRAFT supplier bill, and an exact-owner-approved source-backed Xero DRAFT customer invoice through its dedicated tool. Never authorise or send an invoice, approve a bill, add a payment, reconcile a bank line, change an existing contact, enter bank details, or post a journal.
- If records conflict, flag the conflict and stop short of choosing which financial record to alter, except for the explicitly approved one-cent issued-invoice net reconciliation rule above.
