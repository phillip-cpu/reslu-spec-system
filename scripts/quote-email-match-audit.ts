...
import { buildQuoteThreadMatch, matchQuoteProject, type QuoteEmail, type QuoteProject, type QuoteContact, type QuoteCostLine, type QuoteItem } from "../lib/supplier-quote-email-matching.ts";

const supabase = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.SUPABASE_SERVICE_ROLE_KEY!, { auth: { persistSession: false } });
const [{ data: emails, error: emailError }, { data: projects }, { data: contacts }, { data: sections }, { data: lines }, { data: items }, { data: categories }] = await Promise.all([
  ...
  supabase.from("items").select("id,project_id,item_code,name,description,category,brand,supplier,supplier_contact_id,cost_scope").is("deleted_at", null),
  supabase.from("categories").select("prefix,name"),
]);
...
const categoryNames = new Map((categories ?? []).map((row) => [row.prefix, row.name]));
const quoteItems = (items ?? []).map((row) => ({ ...row, category_name: categoryNames.get(row.category) ?? null })) as QuoteItem[];
...
const results = [...groups.values()].map((thread) => {
  const result = buildQuoteThreadMatch({ emails: thread, projects: (projects ?? []) as QuoteProject[], contacts: (contacts ?? []) as QuoteContact[], lines: costLines, items: quoteItems });
  return {
    ...
    selected_items: result.items.filter((item) => item.selected).map((item) => `${item.item_code} ${item.name}`),
