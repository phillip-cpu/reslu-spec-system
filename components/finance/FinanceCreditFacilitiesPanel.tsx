    ...
    <section className="border border-charcoal/20 bg-offwhite">
      ...
      {showForm && canEdit && (
        <form className="grid gap-4 border-b border-charcoal/20 bg-cream p-5 md:grid-cols-2 xl:grid-cols-4 md:p-7" onSubmit={(event) => { event.preventDefault(); void save(); }}>
          <label className="md:col-span-2"><span className="label-caps">Xero account</span><select required value={form.xeroAccountId} onChange={(event) => { const account = data?.xero_accounts.find((item) => item.id === event.target.value); patch("xeroAccountId", event.target.value); if (account?.bank_account_type === "CREDITCARD") patch("type", "credit_card"); else if (account?.bank_account_type === "LIABILITY") patch("type", "line_of_credit"); else if (form.type === "credit_card") patch("type", "overdraft"); }} className="mt-2 w-full border border-charcoal/20 bg-offwhite px-3 py-2 text-body"><option value="">Select Xero account</option>{(data?.xero_accounts ?? []).filter((account) => !data?.facilities.some((facility) => facility.xero_bank_account_id === account.id && facility.id !== form.id)).map((account) => <option key={account.id} value={account.id}>{account.name} — {account.balance_minor === null ? "Xero balance pending" : `Xero ${formatMinorCurrency(account.balance_minor)}`}</option>)}</select></label>
          <label><span className="label-caps">Type</span><select value={form.type} disabled={Boolean(form.xeroAccountId && data?.xero_accounts.find((item) => item.id === form.xeroAccountId)?.bank_account_type !== "BANK")} onChange={(event) => patch("type", event.target.value as FinanceCreditFacilityType)} className="mt-2 w-full border border-charcoal/20 bg-offwhite px-3 py-2 text-body disabled:opacity-60">{TYPES.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
      ...
      <div className="divide-y divide-charcoal/10 border-t border-charcoal/20">
        {(data?.facilities ?? []).map((item) => <button key={item.id} type="button" disabled={!canEdit} onClick={() => { setForm(fromFacility(item)); setShowForm(true); }} className="grid w-full gap-2 px-5 py-4 text-left hover:bg-cream sm:grid-cols-[1fr_auto] sm:items-center"><span><span className="block text-body text-nearblack">{item.xero_account_name}</span><span className="text-caption text-charcoal/50">{item.xero_balance_minor === null ? "Xero balance pending sync" : `Xero balance ${formatMinorCurrency(item.xero_balance_minor)}`} · {TYPES.find(([value]) => value === item.facility_type)?.[1]}</span></span><span className="text-right"><span className="block text-subhead text-nearblack">{formatMinorCurrency(item.credit_limit_minor)} limit</span><span className="text-caption text-charcoal/50">{item.xero_balance_minor === null ? "Not counted until Xero sync" : `${formatMinorCurrency(item.available_credit_minor)} available`}</span></span></button>)}
