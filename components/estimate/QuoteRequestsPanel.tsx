...
export function QuoteRequestsPanel({ projectId, estimate, onEstimateReload, initialLineIds = [] }: { projectId: string; estimate: EstimateResponse | null; onEstimateReload: () => void; initialLineIds?: string[] }) {
  ...
  function chooseExistingEmail(email: ExistingQuoteEmail) {
    ...
    else setLineIds(new Set());
    if (email.suggested_item_ids.length > 0) setItemIds(new Set(email.suggested_item_ids));
    else setItemIds(new Set());
  ...
  async function createAndSend(event: React.FormEvent) {
    ...
    try {
      ...
      setTitle(""); setScope(""); setRequestedDate(""); setLineIds(new Set()); setItemIds(new Set()); setContactIds(new Set()); setFiles([]); setExistingEmailId("");
      await load();
      onEstimateReload();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={createAndSend} className="space-y-5 border border-nearblack bg-offwhite p-5">
        ...
        {sourceMode === "new" && <fieldset><div className="mb-2 flex items-end justify-between gap-3"><div><legend className="label-caps">FF&amp;E items</legend><p className="mt-1 text-caption text-charcoal/55">Optional direct-purchase items. Supplier pricing flows back into their trade price and the Finance forecast.</p></div><span className="text-caption text-sand">{itemIds.size} selected</span></div><input type="search" value={itemSearch} onChange={(event) => setItemSearch(event.target.value)} placeholder="Search item code, product or supplier" className="mb-2 w-full border border-[#c9c2b4] bg-white px-3 py-2 text-body" /><div className="max-h-64 overflow-y-auto border border-[#dcd6cc] bg-white">{filteredItems.map((item) => <label key={item.id} className="flex gap-3 border-b border-[#e5e0d6] px-3 py-2 text-body last:border-0"><input type="checkbox" checked={itemIds.has(item.id)} onChange={() => toggle(setItemIds, item.id)} /><span><span className="block text-nearblack">{item.item_code} · {item.name}</span><span className="text-caption text-charcoal/50">{[item.category, item.brand, item.supplier].filter(Boolean).join(" · ")}</span></span></label>)}{filteredItems.length === 0 && <p className="p-4 text-body text-charcoal/50">No direct-purchase FF&amp;E items match that search.</p>}</div></fieldset>}
        ...
        <button disabled={creating || !title.trim() || lineIds.size + itemIds.size === 0 || contactIds.size === 0 || (sourceMode === "existing" && !existingEmailId)} className="bg-nearblack px-5 py-2.5 text-subhead text-white disabled:opacity-40">{creating ? (sourceMode === "existing" ? "Linking thread…" : "Creating and sending…") : sourceMode === "existing" ? "Link email thread to project items" : `Create and send ${contactIds.size || ""} request${contactIds.size === 1 ? "" : "s"}`}</button>
      </form>
