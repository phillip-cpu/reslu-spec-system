-- Project conversations are a shared project workspace. The original scoped
-- conversation RPC returned the existing thread only to users who were
-- already participants, which meant the second staff member to open a
-- project's Messages tab received "conversation scope already exists".
--
-- Keep lead conversations member-scoped, but atomically join an authenticated
-- staff member to an existing project conversation before returning it. The
-- advisory lock already serialises this project+purpose lookup, and the
-- participant unique index makes the join idempotent.
